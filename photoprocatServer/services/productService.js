...
class productServices {
  async createProduct(
    ...
    try {
      ...
      if (
        typeInformations.length !== 0 ||
        Object.keys(JSON.parse(information)).length <
          Object.keys(JSON.parse(type.informations)).length
      ) {
      ...
      for (const e of JSON.parse(information)) {
        const info = new Information({
          ...
          description: Object.entries(e)[0][1],
        });
        await info.save();
  ...
  async getAll(query) {
    try {
      ...
      const responseForInformations = await Product.find({
        type: typeId,
      }).populate(['type', 'brand', 'ratings', 'information']);
      responce = await Product.find({
      ...
      if (sort == 'rating') {
        sortNumber == -1
          ? responce.sort(
              (a, b) =>
                b.ratings.reduce((sum, val) => val.rate + sum, 0) -
                a.ratings.reduce((sum, val) => val.rate + sum, 0)
            )
          : responce.sort(
              (a, b) =>
                a.ratings.reduce((sum, val) => val.rate + sum, 0) -
      ...
      responce = [
        ...responce.filter((e) => {
          return checkedBrands.length == 0
            ? e
            : checkedBrands.includes(e.brand._id.valueOf());
        }),
      ];
          ...
          ...responce.filter((e) => {
            ...
            for (const it of e.information) {
              ...
              if (typeInformation[it.name] == 'radio') {
                if (
                  !Object.entries(informations).find(
                    (f) => f[0] === it.name && f[1] === it.description
                  ) &&
                  !Object.entries(informations).find((f) => f[1] === 'неважно')
                ) {
                  bool = false;
                }
              }
              ...
              if (typeInformation[it.name] == 'check') {
                ...
                if (
                  !Object.entries(informations).find(
                    (f) => f[0] === it.name && f[1].includes(it.description)
                  ) &&
                  informations[it.name].length !== 0
                ) {
                  bool = false;
                }
              }
              ...
              if (typeInformation[it.name] == 'slider') {
                if (
                  !Object.entries(informations).find(
                    (f) =>
                      f[0] === it.name &&
                      Number(it.description) >= Number(f[1][0]) &&
                      Number(it.description) <= Number(f[1][1])
                  )
                ) {
                  bool = false;
                }
  ...
  async createMany() {
    try {
      ...
      for (const product of products) {
        const { name, price, description, information, typeName, imagesPath } =
          product;
        const date = Date.now();
