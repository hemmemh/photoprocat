...
class compareItemServices {
  async createCompareItem(compareId, product) {
    try {
      const compare =
        await Compare.findById(compareId).populate('compareItems');
      if (!compare) {
      ...
      if (!compare.compareItems.find((e) => e.product.toString() == product)) {
        ...
        const item = await CompareItem.findOne({ _id: response._id }).populate({
          path: 'product',
          populate: [
            { path: 'type' },
            { path: 'brand' },
  ...
  async deletecompareItem(id, compareId) {
    try {
      const compare = await Compare.findById(compareId).populate({
        path: 'compareItems',
        populate: {
          path: 'product',
          populate: [
      ...
      compare.compareItems = compare.compareItems.filter(
